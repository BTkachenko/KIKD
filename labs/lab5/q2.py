...
def find_best_bit_allocation(total_bits, original, criterion):
    """Znajduje najlepszy podział bitów na poszczególne kanały kolorów."""
    best_allocation = None
    if criterion == "SNR":
        best_snr = -np.inf
        for r_bits in range(total_bits + 1):
            for g_bits in range(total_bits - r_bits + 1):
                b_bits = total_bits - r_bits - g_bits
                quantized = np.stack([
                    quantize_channel(original[:, :, 0], b_bits),
                    quantize_channel(original[:, :, 1], g_bits),
                    quantize_channel(original[:, :, 2], r_bits)], axis=2)
                snr_values = [snr(original[:, :, i], calculate_mse(original[:, :, i], quantized[:, :, i]))
                              for i in range(3)]
                min_snr = min(snr_values)
                if min_snr > best_snr:
                    best_snr = min_snr
                    best_allocation = (r_bits, g_bits, b_bits)
    elif criterion == "MSE":
        best_mse = np.inf
        for r_bits in range(total_bits + 1):
            for g_bits in range(total_bits - r_bits + 1):
                ...
                quantized = np.stack([
                    ...
                    quantize_channel(original[:, :, 1], g_bits),
                    quantize_channel(original[:, :, 2], r_bits)], axis=2)
                mse_values = [calculate_mse(original[:, :, i], quantized[:, :, i])
                              for i in range(3)]
...
def main(input_image_path, output_image_path, total_bits, criterion):
    original = read_tga(input_image_path)

    # Dobór podziału bitów
    r_bits, g_bits, b_bits = find_best_bit_allocation(total_bits, original, criterion)

    # Kwantyzacja z wybranym podziałem bitów
    quantized = np.stack([
        quantize_channel(original[:, :, 0], b_bits),
        quantize_channel(original[:, :, 1], g_bits),
        quantize_channel(original[:, :, 2], r_bits)], axis=2)
    ...
    print(f"Bit allocation: R={r_bits}, G={g_bits}, B={b_bits}")
...
if __name__ == "__main__":
    ...
    main(input_path, output_path, total_bits, criterion)
